...
class GenerateConfigCli():
    ...
    @click.command()
    @click.option('-d', '--dest-asn', type=int, help='Destination ASN for traffic')
    @click.option('-i', '--dest-ixp', help='Destination IXP for traffic')
    @click.option('-t', '--template', help='The template to use for config generation')
    @click.pass_obj
...
class GenerateConfig(PeermeCmd):
    ...
    def _template_render(self, template, peer):
        ...
        return template.render(
            my_asn=self.opts.config.config['peerme']['my_asn'], peer=peer
        )
